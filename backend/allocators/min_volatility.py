...
class MinVolatilityAllocator(Allocator):
    """
    ...
    async def compute(
        ...
        """
        Computes the portfolio allocations using MinVolatility optimization.
        ...
        if not self._update_enabled:
            ...
            if progress_callback:
                await progress_callback(f"MinVolatility optimization complete for {self._name}", 1, 1)
        ...
        while current_date < test_end_date:
            ...
            if progress_callback:
                await progress_callback(
                    f"Optimizing MinVolatility for {self._name} (segment {segment_count})...",
                    segment_count,
                    segment_count + 1
                )
        ...
        if progress_callback:
            await progress_callback(
                f"MinVolatility optimization complete for {self._name} ({segment_count} segments)",
                segment_count,
                segment_count
            )
