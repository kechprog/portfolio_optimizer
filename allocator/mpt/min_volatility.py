    ...
    def configure(cls: Type['MinVolatilityAllocator'],
        ...
        if dialog.result_is_ok:
            new_state: AllocatorState = {
                ...
                "target_return_value": dialog.result_target_return_value,
...
class MinVolatilityConfigDialog(simpledialog.Dialog):
    ...
    def _toggle_target_return_entry_state(self, *args) -> None:
        state = "normal" if self.set_target_return_var.get() else "disabled"
        if self.target_return_entry: self.target_return_entry.configure(state=state)
        if self.target_return_label: self.target_return_label.configure(state=state)
    ...
    def body(self, master_frame: tk.Frame) -> tk.Entry | None:
        ...
        target_return_frame.pack(side="top", fill="x", padx=(20, 0), pady=(0,5))
    ...
    def validate(self) -> bool:
        ...
        if self.result_set_target_return:
            ...
            if not trv_str:
                messagebox.showerror("Validation Error", "Target Return cannot be empty if checked.", parent=self)
                return False
            try:
                self.result_target_return_value = float(trv_str)
            except ValueError:
                messagebox.showerror("Validation Error", "Target Return must be a valid number.", parent=self)
                return False
        else:
            self.result_target_return_value = None
